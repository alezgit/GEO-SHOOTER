  // ============ AUDIO SYSTEM ============
      ...
      function restoreAudioState() {
        ...
        if (typeof state.muted === 'boolean') {
          ...
          document.getElementById('muteBtn').textContent = state.muted ? 'MUTED' : 'SOUND';
        }
      ...
      function resumeAudioOnInteraction() {
        if (!shouldResumeAudio || audioManager.isMuted || !audioManager.bgMusic) return;
      ...
      const CHARACTERS = {
        TANK: {
          ...
          ability: { name: "Shield", icon: "", cooldown: 20000, duration: 3000, effect: "shield" }
        },
        SNIPER: {
          name: "Sniper",
          color: "#2196F3",
          stats: { speed: 5, health: 80, damage: 60, fireRate: 1.6, bulletSpeed: 15 },
          ability: { name: "Piercing Shot", icon: "", cooldown: 12000, duration: 0, effect: "pierce" }
        },
        RUNNER: {
          name: "Runner",
          color: "#FFC107",
          stats: { speed: 7, health: 70, damage: 15, fireRate: 0.3, bulletSpeed: 10 },
          ability: { name: "Dash", icon: "", cooldown: 8000, duration: 300, effect: "dash" }
        },
        BALANCED: {
          name: "Balanced",
          color: "#9C27B0",
          stats: { speed: 5.5, health: 100, damage: 25, fireRate: 0.5, bulletSpeed: 10 },
          ability: { name: "Heal Burst", icon: "", cooldown: 15000, duration: 0, effect: "heal" }
        },
        ASSAULT: {
          name: "Assault",
          color: "#FF5722",
          stats: { speed: 6, health: 90, damage: 30, fireRate: 0.5, bulletSpeed: 12 },
          ability: { name: "Bullet Storm", icon: "", cooldown: 12000, duration: 0, effect: "storm" }
        }
      };
      ...
      const DROP_TYPES = {
        ABILITY: { icon: "⚡", color: "#FFD700", effect: "recharge_ability", duration: 12000, radius: 20 },
        SPEED: { icon: "💨", color: "#00BCD4", effect: "speed_boost", duration: 10000, radius: 20, boostDuration: 5000 },
        HEALTH: { icon: "❤️", color: "#4CAF50", effect: "heal", duration: 8000, radius: 20, healAmount: 30 }
      };

      const ARENA_EVENTS = {
        INFERNO: {
          name: "Fire Tempest", icon: "", duration: 15000,
          backgroundTint: 'rgba(255, 69, 0, 0.25)', wallColor: '#8B0000', gridColor: 'rgba(255, 100, 0, 0.15)'
        },
        BLIZZARD: {
          name: "Ice Blizzard", icon: "", duration: 18000,
          backgroundTint: 'rgba(0, 191, 255, 0.2)', wallColor: '#4682B4', gridColor: 'rgba(200, 230, 255, 0.1)', particleColor: '#E0F7FF'
        },
        GRAVITY_CHAOS: {
          name: "Gravity Chaos", icon: "", duration: 8000,
      ...
      function updateScoreboard() {
        ...
        sortedPlayers.forEach((player, i) => {
          ...
          const livesDots = player.lives > 0
            ? Array.from({ length: player.lives }, () => '<span class="life-dot"></span>').join('')
            : '-';
          const li = document.createElement('li');
          li.className = player.lives <= 0 ? 'eliminated' : '';
          
          li.innerHTML = `
            <span class="player-name" style="color: ${player.color || char.color}">${i + 1}. ${displayName}</span>
            <span class="player-stats" style="color: ${player.color || char.color}">${livesDots}</span>
          `;
...
// -----------------------------
      ...
      function initCustomMapSelect() {
        ...
        if (!mapSelect || !mapSelectList || !mapSelectLabel || !mapSelectButton) return;
        ...
        document.addEventListener('click', (event) => {
          if (!mapSelectButton.contains(event.target) && !mapSelectList.contains(event.target)) {
            mapSelectList.classList.remove('open');
            mapSelectButton.setAttribute('aria-expanded', 'false');
          }
        });
      ...
      document.getElementById("mapSelect").onchange = (e) => {
        ...
        if (mapSelectLabel && e.target.selectedIndex >= 0) {
          mapSelectLabel.textContent = e.target.options[e.target.selectedIndex].textContent;
        }
        ...
        switch (data.type) {
          ...
          case "player_action":
            ...
            if (data.action === 'blow' && player.onFire) {
              ...
              ws.send(JSON.stringify({ type: 'player_status', playerId: player.id, status: 'clear', message: 'Fire extinguished.' }));
            }
            
            if (data.action === 'shake' && player.frozen) {
              delete player.frozen;
              delete player.blizzardExposure;
              ws.send(JSON.stringify({ type: 'player_status', playerId: player.id, status: 'clear', message: 'Broke free from ice.' }));
            }
            break;
          ...
          case "storm": {
            ...
            for (let i = 0; i < bulletCount; i++) {
              ...
              bullets.push({
                ...
                owner: playerId, damage: char.stats.damage, color: (player.color || char.color), pierce: false
              });
            }
      ...
      function shoot(playerId) {
        ...
        bullets.push({
          ...
          owner: playerId, damage: char.stats.damage, color: playerColor, pierce: isPiercing
        });
          ...
          if (currentEvent) {
            if (currentEvent.type === 'INFERNO') {
              ...
              if (touchingWall && !player.onFire) {
                ...
                ws.send(JSON.stringify({ type: 'player_status', playerId: player.id, status: 'on_fire', message: 'YOU ARE ON FIRE! BLOW TO EXTINGUISH!' }));
              }
            }
            ...
            if (currentEvent.type === 'BLIZZARD') {
              ...
              if (!protectedByWall) {
                ...
                if (player.blizzardExposure > 180 && !player.frozen) {
                  ...
                  ws.send(JSON.stringify({ type: 'player_status', playerId: player.id, status: 'frozen', message: 'YOU ARE FROZEN! SHAKE TO BREAK FREE!' }));
                }
      ...
      function draw() {
        ...
        specialZones.forEach(zone => {
          ...
          const icon = zone.type === "speed" ? "⚡" : zone.type === "damage" ? "☠️" : zone.type === "heal" ? "💚" : "🌀";
